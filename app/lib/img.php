...
Class img {
    ...
    function create() { //生成缩略图
        ...
        if (!$this->input_type)
            $this->get_type();
        if (!$this->output_type)
            $this->output_type = $this->input_type;

        if ($this->input_type == "jpg")
            $this->input_type = "jpeg";
        if ($this->output_type == "jpg")
        ...
        if ($this->save_file) {
            switch ($this->output_type) {
                ...
                case 'jpeg':
                    if (APP_SERVER == 'sae') {
                        $src_img = ImageJPEG($dst_img, SAE_TMP_PATH . $this->save_file, $this->jpeg_quality);
                    } else {
                ...
                case 'png':
                    if (APP_SERVER == 'sae') {
                        $src_img = ImagePNG($dst_img, SAE_TMP_PATH . $this->save_file);
                    } else {
                        $src_img = ImagePNG($dst_img, $this->save_file);
                    }
                    break;
                ...
                default:
                    if (APP_SERVER == 'sae') {
                        $src_img = ImagePNG($dst_img, SAE_TMP_PATH . $this->save_file, $this->jpeg_quality);
                    } else {
                        $src_img = ImageJPEG($dst_img, $this->save_file, $this->jpeg_quality);
                    }
                    break;
