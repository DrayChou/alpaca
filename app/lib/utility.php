...
// 跳转函数
function redirect($url, $msg = '', $ext_msg = '')
{ //跳转
    //echo $msg;
...
// 生成 select options
function select_option($list, $value = FALSE)
{ // 利用数组 生成 selecct 的 Opitons
    foreach ($list as $k => $v) {
...
{
    global $uri;
    $file = (APP_SERVER == 'sae' ? SAE_TMP_PATH : APP . 'cache/') . md5($uri);
