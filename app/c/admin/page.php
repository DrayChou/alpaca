...
class page extends admin{
  ...
  function add( $rel_id = 0)
  {
    ...
    if ( $err === TRUE) {
      ...
      if( $_POST['mod'] == 'dir' )  $this->cachedir();
      ...
      $modid = 1;
      if(isset($pinfo['child_model'])&& is_numeric($pinfo['child_model'])) $modid = $pinfo['child_model'];
      if(isset($_GET['model'])&& is_numeric($_GET['model'])) $modid = $_GET['model'];
  ...
  function edit($id)
  {
    ...
    if ( $err === TRUE) {
      ...
      $modid = 1;
      if(isset($pinfo['child_model'])&& is_numeric($pinfo['child_model'])) $modid = $pinfo['child_model'];
      if(isset($info['model'])&& is_numeric($info['model'])) $modid = $info['model'];
      if(isset($_GET['model'])&& is_numeric($_GET['model'])) $modid = $_GET['model'];
      $_POST['info']['model'] = $param['modid'] = $modid;
...
/* 菜单链接 */
  ...
  function arrange($arr)
  {
    ...
    { 
      for($j=$len-1;$j>=$i;$j--)
      {
        if(!isset($arr[$j-1]) || !isset($arr[$j]['order']))continue;
...
function admin_breadcrumb($id = 'current' )
{
  global $pid;
  $c = '';
  if($id == 'current') $id = $pid; 
  if($id == 0 )return '<a href="'.BASE.'admin/page/" >根目录</a> » ';
  $r = load('m/elem_m')->get($id);
  if($id != $pid) $c = ' <a href="'.BASE.'admin/page/index/'.$r['id'].'/" >'.$r['title'].'</a> » ';
