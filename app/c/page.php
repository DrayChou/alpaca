...
class page extends base{
  ...
  function view( $url = 'home' , $action = 'view' )
  {
    ...
    $pid    = $page['id'];
    $rel_id =  $page['rel_id'];
    if($rel_id) $page['parent'] = $this->m->get($rel_id);
    ...
    if(isset($info['page_size']) && $info['page_size'] > 0 ) {
      ...
      if(isset($info['child_dir'])  && $info['child_dir'] > 0  ) $mods[] = '\'dir\'';
    ...
    view($template,$param);
    if(is_numeric(alpa('cache_time'))&& alpa('cache_time') > 0)cache_end();
  }
...
/* extends functions  */
  ...
  {
    if(!$rel_id)redirect('/','对不起，您没有权限');
    $parent = $this->m->get( $rel_id );
    $info = _decode($parent['elem_info']);
    if($this->u['level'] < $info['user_add'] )redirect('/','对不起，您没有权限');
