...
<script>
    $(function () {
        ...
        $('.once').one("focus", function () {
            $(this).val('');
        });

        $(".input-ename").focus(function () {
...
<form method="POST">
    ...
    <table cellspacing="0">
        ...
        <tr>
            ...
            <td><input type="text" name="title_label"
                       value="<?= isset($val['title_label']) ? $val['title_label'] : '标题' ?>"/></td>
                    ...
                    <td>
                        <select name="fields[<?= $i ?>][model]" class="input-type">
                            <?php foreach ($option as $k => $v) { ?>
                                <option
                                    value="<?= $k ?>" <?= $l['model'] == $k ? 'selected="TRUE"' : '' ?>><?= $v ?></option> <?php } ?>
                        </select>
                        <textarea <?= ($l['model'] == 'radio' || $l['model'] == 'checkbox') ? '' : 'style="display:none;"' ?>
                            class="input-ename"
                            name="fields[<?= $i ?>][enum]"><?= isset($l['enum']) ? $l['enum'] : "选项1\n选项2\n选项3" ?></textarea>
                    </td>
                    <td><input type="text" size="4" name="fields[<?= $i ?>][order]"
                               value="<?= isset($l['order']) ? $l['order'] : '' ?>"/></td>
                    <td><a onclick="$(this).parent().parent().remove();" title="删除"><img
                                src="<?= BASE ?>static/img/b_drop.png"/></a></td>
                </tr>
            <?php
            }
        }
        ...
        <?php for ($j = $i + 1; $j < $i + 8; $j++): ?>
            <tr <?= ($j % 2) ? '' : ' class="odd"' ?> >
                ...
                <td>
                    ...
                    <textarea style="display:none;" name="fields[<?= $j ?>][enum]" class="input-ename">
                        选项1
                        选项2
                        选项3
                    </textarea>
                ...
                <td><input type="text" size="4" name="fields[<?= $j ?>][order]" value=""/></td>
                <td></td>
            </tr>
        <?php endfor; ?>
        <tr>
            <th></th>
            <td><input type="submit" class="submit-button" value="更新"></td>
        </tr>
    </table>
</form>
