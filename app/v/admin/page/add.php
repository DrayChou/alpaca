<script>
    ...
    var onUploadCallback = function (sText, bFinish, fromfile) {
...
<div class="col75">
    ...
    <div>
        <input type="text" name="title" placeholder="标题" value="<?= isset($val['title']) ? $val['title'] : '' ?>"
               id="input_title">
...
<div class="col25 sidebar">
    ...
    <div class="box" id="first">
        ...
        <div>发布时间</div>
        <div>
            <input type="text" name="post_time"
                   value="<?= isset($val['post_time']) ? date('Y-m-d H:m:s', $val['post_time']) : date('Y-m-d H:m:s') ?>"/>
        </div>
    </div>
    ...
    <div class="box">
        ...
        <div>网页标题</div>
        <div>
            <input type="text" name="info[page_title]"
                   value="<?= isset($val['info']['page_title']) ? $val['info']['page_title'] : '' ?>">
        </div>
        <div>meta:keywords</div>
        <div>
            <input type="text" name="info[meta_keywords]"
                   value="<?= isset($val['info']['meta_keywords']) ? $val['info']['meta_keywords'] : '' ?>">
        </div>
        <div>meta:description</div>
        <div>
            <input type="text" name="info[meta_description]"
                   value="<?= isset($val['info']['meta_description']) ? $val['info']['meta_description'] : '' ?>">
        </div>
    </div>
    <h4>模板排版</h4>
    ...
    <div class="box">
        <div>风格</div>
        <div>
            <input name="info[template]" type="text"
                   value="<?= isset($val['info']['template']) ? $val['info']['template'] : '' ?>">
        </div>

        <div>排版</div>
        <div>
            <input name="info[layout]" type="text"
                   value="<?= isset($val['info']['layout']) ? $val['info']['layout'] : '' ?>">
        </div>
    </div>
    ...
    <div class="box">
        <div>
            <input type="checkbox" value="1"
                   name="info[dir]" <?= isset($val['info']['dir']) && $val['info']['dir'] ? 'checked="true"' : '' ?> />
            栏目页
            <br/>
            每页显示 <input class="text" style="width:20px;"
                        value="<?= isset($val['info']['page_size']) ? $val['info']['page_size'] : '20' ?>" size="3"
                        type="text" name="info[page_size]"/> 条
            <br/>
            <input type="checkbox" name="info[child_dir]"
                   value="1" <?= isset($val['info']['child_dir']) && $val['info']['child_dir'] ? 'checked="true"' : '' ?> />
            显示子栏目
            <br/>
            <input type="checkbox" name="info[grandchild]"
                   value="1" <?= isset($val['info']['grandchild']) && $val['info']['grandchild'] ? 'checked="true"' : '' ?> />
            显示孙页面
        </div>
    </div>
    <h4> 子页面设置</h4>
    ...
    <div class="box">
        <ul class="admin_list_menu">
            ...
            <li>
                默认风格
                <input type="text" name="info[child_template]"
                       value="<?= isset($val['info']['child_template']) ? $val['info']['child_template'] : '' ?>"/>
            </li>

            <li>
                默认排版
                <input type="text" name="info[child_layout]"
                       value="<?= isset($val['info']['child_layout']) ? $val['info']['child_layout'] : '' ?>"/>
            </li>
            ...
            <li>
                ...
                <select name="info[child_model]">
                    <?php foreach ($model as $v) { ?>
                        <option
                            value="<?= $v['id'] ?>" <?= isset($val['info']['child_model']) && $val['info']['child_model'] == $v['id'] ? ' selected ' : '' ?>><?= $v['title'] ?></option>
                    <?php } ?>
                </select>
            </li>
        </ul>
        ...
        <ul class="admin_list_menu">
            ...
            <li>
                浏览 <select name="info[user_browse]">
                    ...
                    <?php for ($i = 0; $i < 11; $i++) { ?>
                        <option
                            value="<?= $i ?>" <?= isset($val['info']['user_browse']) && $val['info']['user_browse'] == $i ? ' selected ' : '' ?>><?= $i ?></option>
                    <?php } ?>
                </select>
            </li>
            ...
            <li>
                添加 <select name="info[user_add]">
                    ...
                    <?php for ($i = 0; $i < 11; $i++) { ?>
                        <option
                            value="<?= $i ?>" <?= isset($val['info']['user_add']) && $val['info']['user_add'] == $i ? ' selected ' : '' ?>><?= $i ?></option>
                    <?php } ?>
                </select>
            </li>
            <!--
