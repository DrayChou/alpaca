...
<head>
    ...
    <script>
        ...
        var del = function (id) {
            ...
            if (!ret)return;
            $.post('<?=BASE?>admin/elem/del/' + id, function () {
        ...
        var mdel = function () {
            ...
            $.post('<?=BASE?>admin/page/mdel/', sarr, function () {
                window.location.reload();
            });
        }
        ...
        var move = function () {
            ...
            if (!ret)return;
            sarr = $(".sid:checked").serialize();
            $.post('<?=BASE?>admin/page/move/' + moveto + '/', sarr, function () {
                window.location.reload();
            });
        }
    </script>
...
<div id="menu">
    <ul>
        ...
        foreach ($menu as $url => $label) {
            ?>
            <li <?=$url == seg(1) ? ' class="hover" ' : ''?> ><a
                    href="<?= BASE ?>admin/<?= $url ?>/index/"><?=$label?></a></li>
        <?php
        }
        ?>
    ...
    <div id="submenu">
        <ul>
            ...
            foreach ($submenu as $url => $label) {
                ?>
                <li <?=$url == seg(2) ? ' class="hover" ' : ''?> ><a
                        href="<?= BASE ?>admin/<?= seg(1) ?>/<?= $url ?>/"><?=$label?></a></li>
            <?php
            }
            ?>
        </ul>
