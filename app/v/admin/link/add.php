<form method="POST">
    ...
    <table cellspacing="0">
        <tr>
            ...
            <td>
                <span class="error"><?= isset($err['title']) ? $err['title'] : '' ?></span></td>
            <td><input type="submit" class="submit-button" value="更新菜单">
            </td>
        </tr>
    </table>
    <table cellspacing="0">
        <tr class="th">
            ...
            <th>url地址</th>
            <th>排序</th>
            <th>新窗口 <a title="选中后，链接将在新页面打开" class="tip">?</a></th>
            <th></th>
        </tr>
        <?php
        $i = 0;
        if (isset($links)) {
            foreach ($links as $l) {
                ...
                <tr <?= ($i % 2) ? ' class="odd"' : '' ?> >
                    ...
                    <td><input type="text" size="30" name="info[<?= $i ?>][link]"
                               value="<?= isset($l['link']) ? $l['link'] : '' ?>"/></td>
                    <td><input type="text" size="4" name="info[<?= $i ?>][order]"
                               value="<?= intval($l['order']) ? $l['order'] : '' ?>"/></td>
                    <td><input name="info[<?= $i ?>][blank]" value="1"
                               type="checkbox" <?= intval($l['blank']) ? 'checked="TRUE"' : '' ?> /></td>
